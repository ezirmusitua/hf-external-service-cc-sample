...
## Call the script to isntall and instantiate a chaincode on the channel
function deployAnyCC() {

  scripts/deployAnyCC.sh $CHANNEL_NAME $CC_NAME $CC_SRC_LANGUAGE $CC_SRC_PATH $VERSION $CLI_DELAY $MAX_RETRY $VERBOSE
...
function callCC() {
  ...
  scripts/callCC.sh $CHANNEL_NAME $CC_NAME $FN_NAME $FN_ARGS $IS_INIT

  if [ $? -ne 0 ]; then
    echo "ERROR !!! Call chaincode function failed"
    exit 1
  fi
...
if [ "$MODE" == "up" ]; then
  ...
  echo
elif [ "$MODE" == "deployAnyCC" ]; then
  echo "deploying any chaincode ${CC_NAME} on channel '${CHANNEL_NAME}'"
  echo
elif [ "$MODE" == "callCC" ]; then
  echo "call chaincode ${CC_NAME} function ${FN_NAME} with ${FN_ARGS} on channel '${CHANNEL_NAME}'"
  echo
else
  printHelp
  exit 1
